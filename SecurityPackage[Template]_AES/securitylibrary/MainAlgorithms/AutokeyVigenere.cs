...
namespace SecurityLibrary
{
    public class AutokeyVigenere : ICryptographicTechnique<string, string>
    {
        ...
        public string Decrypt(string cipherText, string key)
        {
            ...
            int cntr_plan = 0;
            int cntr_cip = key.Length;
            while (P_T.Length < cipherText.Length)
            {
                int indx = (find_ch.IndexOf(cipherText[cntr_cip]) - find_ch.IndexOf(P_T[cntr_plan])) + 26;
                indx = indx % 26;
                P_T += find_ch[indx];
                cntr_cip++;
                cntr_plan++;
